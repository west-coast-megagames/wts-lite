...
import type { InputProps, TagRootProps, TagLabelProps, TagCloseTriggerProps, WrapItemProps, WrapProps } from '@chakra-ui/react'
...
type MaybeTagRootProps<P> = MaybeFunc<[tag: string, index?: number], P>
...
export type ChakraTagInputProps = InputProps & {
  ...
  tagRootProps?: MaybeTagRootProps<TagRootProps>
  tagLabelProps?: MaybeTagRootProps<TagLabelProps>
  tagCloseTriggerProps?: MaybeTagRootProps<TagCloseTriggerProps>
}

export default forwardRef(function TagInputGroup({
  tags = [],
  onTagsChange,
  onTagAdd,
  onTagRemove,
  ...
  return (
    <Wrap align="center" {...wrapProps}>
      {tags.map((tag, index) => (
        <WrapItem {...maybeCall(wrapItemProps, false, index)} key={index}>
          <ChakraTagInputTag
            ...
            tagCloseButtonProps={maybeCall(tagCloseTriggerProps, tag, index)}
            colorScheme={props.colorScheme}
            {...maybeCall(tagRootProps, tag, index)}
          >
            {tag}
          </ChakraTagInputTag>
        </WrapItem>
      ...
      <WrapItem flexGrow={1} {...maybeCall(wrapItemProps, true, tags.length)}>
        <Input size={'xs'} {...props} onKeyDown={handleKeyDown} ref={ref}/>
      </WrapItem>
    </Wrap>
  )
})